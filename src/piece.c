...
struct piece piece_create(const enum piece_type type)
{
    ...
    for (uint16_t i = 0; i < PIECE_NUM_ROTATIONS; i++)
    {
        for (uint16_t j = 0; j < PIECE_NUM_SQUARES; j++)
        {
    ...
    for (uint16_t i = 0; i < PIECE_NUM_ROTATIONS; i++)
    {
        for (uint16_t j = 0; j < PIECE_NUM_COLLISION_CHECKS; j++)
        {
...
void piece_rotate(struct piece* piece, const int16_t direction)
{
